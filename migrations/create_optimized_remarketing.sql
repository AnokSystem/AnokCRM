-- Optimistic Fetch & Advance Function for Remarketing (v2)
-- Added SECURITY DEFINER to bypass RLS issues when called via API.
...
LANGUAGE plpgsql
SECURITY DEFINER -- IMPORTANTE: Executa como administrador (bypassa RLS)
...
DECLARE
    ...
    v_has_next BOOLEAN;
BEGIN
    -- Loop through pending enrollements
    FOR r IN 
        SELECT 
        ...
        AND e.next_execution_at <= now() -- Simplified timestamp check
        LIMIT 50
    LOOP
        -- Return this row
